...
The DevConsole package declares \texttt{com.mlgworks.utils} at semantic
version \texttt{1.0.0}. The consuming project must therefore provide the
Git source for Utils as a direct dependency; Unity Package Manager does not
accept a Git URL in another package's dependency field. In the Package
Manager window, install Utils first with \textbf{Add package from git URL},
then install DevConsole using the same option.
